...
<body>
  <div class="card">
    ...
    <p><a class="btn" id="openBtn" href="{{ deep_link }}">Открыть сейчас</a>
       <a class="btn secondary" href="#" onclick="copyLink();return false;">Скопировать ссылку</a></p>
  ...
  <script>
    ...
    function copyLink(){
      const el = document.createElement('textarea'); el.value = target;
...
def open_link():
    ...
    Пример из бота: /open?url=v2raytun%3A%2F%2Fimport%2Fvless%25253A%25252F%25252F...
    """
    url = (request.args.get("url") or "").strip()
    if not url:
        abort(400, "missing url")
    # url может быть сам заэнкоден ботом — НЕ декодируем дважды; передаём как есть в кнопку и JS.
    ...
    return render_template_string(
        HTML_OPEN,
        app_name=APP_NAME,
        deep_link=url,                   # показываем как есть (urlencoded)
        deep_link_js=url,                # и в JS тоже (приложение принимает корректно)
...
def copy_page():
    ...
    Выводит страницу с текстовым полем для копирования вручную.
    """
